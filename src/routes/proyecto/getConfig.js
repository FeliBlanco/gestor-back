...
const getConfig = async (req, res) => {
    try {
        ...
        const result = await clientPS.query(`SELECT P.nombre, P.sistema_docker, P.system_port, P.start_command, P.build_command, P.install_command, P.output_directory, P.repositorio, P.rama, P.framework, F.nombre AS framework, F.id AS framework_id FROM proyectos P LEFT JOIN frameworks F ON F.id = P.framework WHERE P.id = $1`, [project_id])
