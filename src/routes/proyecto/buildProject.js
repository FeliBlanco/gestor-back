...
const buildProject = async (req, res) => {
    ...
    if(tipo_sistema == "front")  {
        ...
        && docker run --rm \
    -v /tmp/build_project2:/build_input:ro \
    -v ${global.URL_PROYECTOS}${grupo.rows[0].usuario}:/output \
    -w /app \
    ${tipo_sistema_docker} \
    sh -c "cp -r /build_input/* . && ${data.install_command} && ${data.build_command} && cp -r ${data.output_directory}/* /output/${data.proyect_directory}"`, async (error, stdout, stderr) => { 
            if(error) {
