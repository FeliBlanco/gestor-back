...
app.get('/actualizar/:id', async (req, res) => {
    ...
    if(tipo_sistema == "front")  {
        ...
        const child = exec(`docker run --rm -v /var/www/${grupo.rows[0].usuario}:/output node:18-alphine sh -c "apk add --no-cache git; git clone ${repositorio} /tmp/build_project2; cd /tmp/build_project2; npm install; npm run build; cp -r dist /output"`, (error, stdout, stderr) => { 
            if(error) {
